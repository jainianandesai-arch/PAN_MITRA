...
INSTANT_EPAN_GUIDE_CONTEXT = """Source: https://www.incometax.gov.in/iec/foportal/help/all-topics/instant-e-pan/faq
Service/form: Instant e-PAN (free, Aadhaar-OTP based) through the official Income Tax e-Filing portal — separate from the paid Protean/UTIITSL new-PAN flow.

CSC guide for Instant e-PAN:
1. Confirm the applicant is an individual with a valid, mobile-linked Aadhaar and does NOT already have a PAN. Instant e-PAN is not for firms/companies/trusts/HUF.
2. Open the official Income Tax e-Filing instant e-PAN page, not the Protean/UTIITSL PAN portal.
3. Select "Get New e-PAN" and enter the Aadhaar number only inside the official portal field. Do not paste Aadhaar or OTP into this chatbot.
4. Complete Aadhaar OTP verification only on the official portal.
5. Review the e-KYC details fetched automatically from Aadhaar (name, DOB, address, gender) — there is no manual data entry and no document upload.
6. Submit. The e-PAN is usually generated within minutes.
7. Download the e-PAN PDF from the official portal once generated, and check status using the Aadhaar number on the official instant e-PAN status page if it is not immediate.

DPDP note: Instant e-PAN needs no documents and no fee — never collect or store Aadhaar/OTP in this chat; enter them only inside the official Income Tax e-Filing portal."""


INSTANT_EPAN_GUIDE_CONTEXT_HI = """Source: https://www.incometax.gov.in/iec/foportal/help/all-topics/instant-e-pan/faq
Service/form: Instant e-PAN (free, Aadhaar-OTP आधारित) official Income Tax e-Filing portal से — यह paid Protean/UTIITSL new-PAN flow से अलग है।

Instant e-PAN के लिए CSC guide:
1. पहले confirm करें कि applicant individual है, valid mobile-linked Aadhaar है, और उसके पास पहले से PAN नहीं है। Instant e-PAN firms/companies/trusts/HUF के लिए नहीं है।
2. Official Income Tax e-Filing instant e-PAN page खोलें, Protean/UTIITSL PAN portal नहीं।
3. "Get New e-PAN" चुनें और Aadhaar number केवल official portal field में भरें। Aadhaar या OTP इस chatbot में न डालें।
4. Aadhaar OTP verification केवल official portal पर पूरा करें।
5. Aadhaar से automatically आई e-KYC details (name, DOB, address, gender) review करें — कोई manual data entry या document upload नहीं है।
6. Submit करें। e-PAN आमतौर पर कुछ minutes में generate हो जाता है।
7. Generate होने के बाद official portal से e-PAN PDF download करें, और अगर तुरंत न मिले तो official instant e-PAN status page पर Aadhaar number से status check करें।

DPDP note: Instant e-PAN के लिए कोई document या fee नहीं चाहिए — Aadhaar/OTP इस chat में कभी collect/store न करें; इन्हें केवल official Income Tax e-Filing portal में भरें।"""
...
def builtin_service_context(query, language="en"):
    ...
    instant_epan_terms = ("instant pan", "instant e pan", "instant e-pan", "e pan", "e-pan", "epan", "free pan", "aadhaar pan")
    instant_epan_terms_hi = ("इंस्टेंट पैन", "आधार पैन")
    if any(term in text for term in instant_epan_terms) or any(term in query for term in instant_epan_terms_hi):
        if language == "hi":
            return INSTANT_EPAN_GUIDE_CONTEXT_HI
        return INSTANT_EPAN_GUIDE_CONTEXT
